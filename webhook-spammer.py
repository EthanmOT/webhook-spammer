...
class webhook_spam:
    def __init__(self):
        self.choice = int(input(f"{Fore.BLUE}Would you like to spam the webhook (1) or delete it? (2){Fore.RESET} >>> "))

        if self.choice not in [1,2]:
            print(f"{Fore.RED}Invalid Choice!") 
            str(input(f"{Fore.BLUE}Press anything to exit..."))
            sys.exit()

        if self.choice == 1:
            self.webhook = str(input(f"{Fore.BLUE}Webhook{Fore.RESET} >>> "))
            ...
            self.message = str(input(f"{Fore.BLUE}Message{Fore.RESET} >>> "))
            self.iterations = int(input(f"{Fore.BLUE}Iterations{Fore.RESET} >>> "))
